...
def validate(args,db_val,cur_epochs, model, loader, device, metrics,snapshot_path):
    ...
    with torch.no_grad():
        ...
        for i in range(1, 2):
            ...
            i, metric_list[i - 1][0], metric_list[i - 1][1], score_iou[i] / len(db_val),
            score_dice[str(i)] / len(db_val)))
...
def trainer_monuseg(args, model, snapshot_path):
    best_train = False
    from datasets.dataset_MoNuSeg import MoNuSeg_dataset, RandomGenerator
    ...
    db_train = MoNuSeg_dataset(base_dir=args.root_path, list_dir=args.list_dir, split="train",
                               transform=transforms.Compose(
                                   [RandomGenerator(output_size=[args.img_size, args.img_size])]))
    db_val = MoNuSeg_dataset(base_dir='./data/MoNuSeg/val_data', list_dir=args.list_dir, split="val_vol")
    ...
    optm = 'adam' #'sgd'
    optimizer = optim.SGD(model.parameters(), lr=base_lr, momentum=0.9, weight_decay=0.0001) if optm == 'sgd' else optim.Adam(model.parameters(), lr=base_lr*0.0001)
    #optimizer = optim.Adam(model.parameters(), lr=base_lr)
    scheduler = torch.optim.lr_scheduler.OneCycleLR(optimizer, max_lr = 0.001, total_steps = args.max_epochs* len(trainloader), pct_start= 0.1)
    writer = SummaryWriter(snapshot_path + '/log')
    iter_num = 0
    ...
    for epoch_num in iterator:
        for i_batch, sampled_batch in enumerate(trainloader):
            ...
            if epoch_num >= int(max_epoch * topo_epoch):
                loss_topo = getTopoLoss(outputs, label_batch)  # TODO new
            loss = 0.4 * loss_ce + 0.6 * loss_dice + 0.0001 * loss_topo if epoch_num >= int(
                max_epoch * topo_epoch) else 0.4 * loss_ce + 0.6 * loss_dice # TODO optimazation 1 int(max_epoch)
            optimizer.zero_grad()
            loss.backward()
            optimizer.step()
            if optm == 'adam':
        ...
        if epoch_num % 1 == 0 and epoch_num >= int(-0.02*max_epoch):
            print("************* Validation *************")
            model.eval()
            val_score = validate(args,db_val,cur_epochs=epoch_num, model=model, loader=val_loader, device=device,
