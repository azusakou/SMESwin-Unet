...
def _getTopoLoss(likelihood_tensor, gt_tensor, topo_size=100):
    """
...
def _get_data(x_input,y_target):
    ...
    return soft_output, _one_hot_encoder(y_target)

def getTopoLoss(likelihood_tensor, gt_tensor, topo_size=100, n_classes=2):
    likelihood_tensor, gt_tensor = _get_data(likelihood_tensor, gt_tensor)
    loss = 0
    for i in range(len(likelihood_tensor)):
        for k in range(n_classes):
            loss += _getTopoLoss(likelihood_tensor[i][k], gt_tensor[i][k], topo_size=100)
    return loss/(len(likelihood_tensor)*n_classes)
