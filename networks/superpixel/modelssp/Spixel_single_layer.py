...
class SpixelNet(nn.Module):
    ...
    def forward(self, x):
        ...
        out_deconv0 = self.deconv0(out2) #out_conv1_1)
...
def SpixelNet1l(input_channel=3, data=None):
    # Model without batch normalization
    model = SpixelNet(input_channel=input_channel,batchNorm=False)
    if data is not None:
        model.load_state_dict(data['state_dict'])
    return model

def SpixelNet1l_bn(input_channel=9, data=None):
    # model with batch normalization
    model = SpixelNet(input_channel=input_channel, batchNorm=True)
    if data is not None:
        model.load_state_dict(data['state_dict'])
    return model